...
export function mountCashflow(app, httpServer, { base = '', serveStatic = true, port = null, store = null } = {}) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('student:saleDecision', ({ teamId, accept } = {}, ack) => { ensureTeamRoom(socket, teamId); ack?.(roomOf(socket)?.saleDecision(teamId, accept) || { ok: false }); });
    socket.on('student:quizAnswer', ({ teamId, choice } = {}, ack) => { ensureTeamRoom(socket, teamId); ack?.(roomOf(socket)?.quizAnswer(teamId, choice) || { ok: false }); });
